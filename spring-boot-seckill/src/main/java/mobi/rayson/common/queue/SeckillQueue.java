...
public class SeckillQueue {
  ...
  // 用于多线程间下单的队列
  private static BlockingQueue<SuccessKilled> blockingQueue = new LinkedBlockingQueue<>(QUEUE_SIZE);
   ...
   *
   * 队列未满时，返回true；
   * 队列满则抛出 IllegalStateException(“Queue full”)异常
   * AbstractQueue put(e) 队列未满时，直接插入没有返回值；
   * 队列满时会阻塞等待，一直等到队列未满时再插入。 offer(e)
